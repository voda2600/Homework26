...
let mutateImage (ans:Form) (i:float)= ans.Paint.Add(fun e -> e.Graphics.DrawImage(createImage(-0.710,0.285,70+Convert.ToInt32(i*4.0),i*2.2), 0, 0))
let ans  = new Form()
ans.Paint.Add(fun e -> e.Graphics.DrawImage(createImage(-0.710,0.28,100,1.0), 0, 0))
ans.Size <- new Size(Convert.ToInt32(width), Convert.ToInt32(height))

for i in 1.0..30.0 do
    mutateImage ans i
ans.Paint.Add(fun e -> e.Graphics.DrawImage(createImage(-0.710,0.285,1000,30.0*2.2), 0, 0))
